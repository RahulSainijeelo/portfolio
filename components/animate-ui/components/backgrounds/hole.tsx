...
function HoleBackground({
  ...
  const setLines = React.useCallback(() => {
    ...
    const dpi = stateRef.current.render.dpi;
    const offCanvas = document.createElement('canvas');
    offCanvas.width = width * dpi;
    offCanvas.height = height * dpi;
  ...
  const drawLines = React.useCallback((ctx: CanvasRenderingContext2D) => {
    if (stateRef.current.linesCanvas && stateRef.current.linesCanvas.width > 0 && stateRef.current.linesCanvas.height > 0) {
      const { width, height } = stateRef.current.rect;
      // Draw into the unscaled context using logical units to match the scaled canvas backing
      ctx.drawImage(stateRef.current.linesCanvas, 0, 0, width, height);
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={cn(
          'absolute top-[-71.5%] left-1/2 z-[3] w-[30%] h-[140%] rounded-b-full blur-3xl opacity-90 mix-blend-plus-lighter [transform:translate3d(-50%,0,0)] [background-position:0%_100%] [background-size:100%_200%]',
